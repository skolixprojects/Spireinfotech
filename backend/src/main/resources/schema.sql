...
-- Phase 8: attachments — draft-anchored (message_id NOT NULL). Only the
-- opaque storage_key (Cloudinary raw public_id in prod) is stored; the
-- raw URL is never persisted or returned — downloads go through the
