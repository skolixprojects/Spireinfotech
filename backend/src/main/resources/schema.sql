...
CREATE TABLE mail_mailbox_entries (
    ...
    account_id   BIGINT      NOT NULL REFERENCES mail_accounts(id),
    message_id   BIGINT      NOT NULL REFERENCES mail_messages(id),
    folder       VARCHAR(20) NOT NULL,   -- VESTIGIAL since Phase 14 (kept non-null); routing is by folder_id
    folder_id    BIGINT      REFERENCES mail_folders(id),  -- Phase 14: the folder this entry lives in
...
CREATE INDEX idx_mail_mbox_acct_folderid ON mail_mailbox_entries(account_id, folder_id);
CREATE INDEX idx_mail_mbox_msg           ON mail_mailbox_entries(message_id);

-- Phase 14: per-account folder tree. Five SYSTEM folders (kind=SYSTEM,
-- system_key=INBOX/SENT/DRAFTS/ARCHIVE/TRASH, parent=null) are seeded per
-- account and are undeletable/unrenamable; CUSTOM folders nest arbitrarily.
-- Unique (account_id, parent_folder_id, name); root-level dup names are also
-- guarded in code (SQL treats NULL parents as distinct). ddl-auto creates this
-- table; a boot backfill seeds system folders + sets mailbox_entries.folder_id
-- from the vestigial enum (idempotent, data-only).
CREATE TABLE mail_folders (
    id               BIGINT       GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    account_id       BIGINT       NOT NULL REFERENCES mail_accounts(id),
    name             VARCHAR(255) NOT NULL,
    parent_folder_id BIGINT       REFERENCES mail_folders(id),
    kind             VARCHAR(20)  NOT NULL,   -- SYSTEM | CUSTOM
    system_key       VARCHAR(20),             -- SYSTEM only: INBOX/SENT/DRAFTS/ARCHIVE/TRASH
    sort_order       INT          NOT NULL DEFAULT 0,
    created_at       TIMESTAMP    NOT NULL DEFAULT now(),
    CONSTRAINT uq_mail_folder_acct_parent_name UNIQUE (account_id, parent_folder_id, name),
    -- One system folder per (account, key); CUSTOM rows have NULL system_key
    -- (distinct) so they're unconstrained — blocks duplicate SYSTEM folders.
    CONSTRAINT uq_mail_folder_acct_systemkey UNIQUE (account_id, system_key)
);
CREATE INDEX idx_mail_folder_acct_parent ON mail_folders(account_id, parent_folder_id);

-- Phase 17: per-account inbox rules, evaluated on inbound delivery only
-- (conditions -> actions decide where a recipient's new message lands + its
-- flags). conditions_json / actions_json are PORTABLE JSON TEXT (parsed via
-- Jackson) — NOT a DB JSON type, NOT child tables — so ddl-auto creates this
-- table on any DB. Conditions: [{field:FROM|TO|CC|SUBJECT|HAS_ATTACHMENT,
-- operator:CONTAINS|EQUALS|IS_TRUE, value}]; actions: [{type:MOVE_TO_FOLDER|
-- MARK_READ|STAR|MARK_IMPORTANT|DELETE, targetFolderId?}]. Evaluation is
-- FAIL-OPEN: any error delivers to Inbox/unread (the engine never drops mail).
-- A MOVE target must be a folder the owner owns; a later-deleted target is
-- skipped at evaluation (no rule rewrite needed).
CREATE TABLE mail_rules (
    id              BIGINT       GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    account_id      BIGINT       NOT NULL REFERENCES mail_accounts(id),
    name            VARCHAR(255) NOT NULL,
    priority        INT          NOT NULL DEFAULT 0,   -- evaluation order, asc
    enabled         BOOLEAN      NOT NULL DEFAULT TRUE,
    match_mode      VARCHAR(8)   NOT NULL DEFAULT 'ALL',  -- ALL | ANY
    stop_processing BOOLEAN      NOT NULL DEFAULT FALSE,
    conditions_json TEXT,        -- JSON array of conditions
    actions_json    TEXT,        -- JSON array of actions
    created_at      TIMESTAMP    NOT NULL DEFAULT now()
);
CREATE INDEX idx_mail_rule_acct         ON mail_rules(account_id);
CREATE INDEX idx_mail_rule_acct_enabled ON mail_rules(account_id, enabled);
