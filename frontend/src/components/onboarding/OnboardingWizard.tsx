  ...
  const dots = Array.from({ length: TOTAL_STEPS }, (_, i) => (
    <span
      ...
      className={`block h-2 rounded-full transition-all ${
        i === step ? "bg-[#0E6B6B] w-6" : "bg-gray-300 w-2"
      }`}
    />
  ));
      ...
      >
        ...
        <div className="px-6 sm:px-10 py-5 border-t border-gray-100 flex items-center justify-between gap-3">
          ...
          {step < TOTAL_STEPS - 1 ? (
            <button
              onClick={() => setStep((s) => s + 1)}
              className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-[#0E6B6B] text-white text-sm font-semibold hover:bg-[#5FA3A3] transition shadow-sm"
            >
            ...
            <button
              ...
              disabled={busy}
              className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-[#0E6B6B] text-white text-sm font-semibold hover:bg-[#5FA3A3] disabled:opacity-50 transition shadow-sm"
            >
        ...
        {steps.map((s) => {
          ...
          return (
            <div key={s.n} className="flex gap-4 items-start">
              <div className="w-10 h-10 rounded-xl bg-[#0E6B6B]/10 text-[#0E6B6B] flex items-center justify-center font-bold shrink-0">
                {s.n}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <Icon size={14} className="text-[#0E6B6B] shrink-0" />
        ...
        {CATEGORY_TILES.map((tile) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={`group flex flex-col items-center gap-2 p-4 rounded-xl border border-gray-200 ${tile.bg} hover:border-[#0E6B6B]/40 hover:shadow-md transition disabled:opacity-50 disabled:cursor-not-allowed`}
            >
